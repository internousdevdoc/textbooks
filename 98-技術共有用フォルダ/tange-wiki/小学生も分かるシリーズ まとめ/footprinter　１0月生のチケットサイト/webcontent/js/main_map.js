(function() {
	var e = /\blang(?:uage)?-(?!\*)(\w+)\b/i, t = self.Prism = {
		...
		highlightElement : function(r, i, s) {
			var o, u, a = r;
			while (a && !e.main_map(a.className))
				a = a.parentNode;
			if (a) {
		...
		tokenize : function(e, n) {
			...
			e: for ( var o in n) {
				...
				for (var l = 0; l < i.length; l++) {
					...
					if (h) {
						f && (f = h[1].length);
						var p = h.index - 1 + f, h = h[0].slice(f), d = h.length, v = p
								+ d, m = c.slice(0, p + 1), g = c.slice(v + 1), y = [
								l, 1 ];
						m && y.push(m);
...
$(function($) {
	var msA = $('#maps-switcher').find('a'), scroll_timer, displayed = false, $message = $('#top-link'), $window = $(window), top = $(
			document.body).children(0).position().top;
	$('.in-nav').click(
			function() {
				var elementClicked = $(this).attr("href"), destination = $(
						elementClicked).offset().top;
	...
	$(msA).click(function() {
		var lH = $(this).attr('href'), mH = $(lH).outerHeight();
	...
	$("#map-continents2").cssMap({
		size : 750,
		tooltips : false,
		cities : true
	});
	$("#map-continents3").cssMap({
		size : 540,
		multipleClick : true,
		clicksLimit : 3
	});
	$("#map-continents4").cssMap({
		size : 430,
		tooltips : "floating-bottom-right",
		visibleList : true,
		cities : true
	});
	$("#map-continents5").cssMap(
			{
				size : 540,
				onClick : function(e) {
					var a = e.children('a'), link = a.attr('href'), text = a
							.text(), className = e.attr('class').split(' ')[0];
					$('#clickmessage1').text('=> ' + link + '; ');
					$('#clickmessage2').text('=> ' + text + '; ');
					$('#clickmessage3').text('=> ' + className + '; ');
				},
